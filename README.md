...
| 🔌 [Plugin de Claude Code](docs/claude-plugin.md) | Las skills curadas (`cimientos`, `spec-flow`, `escritura-tecnica`) instalables a nivel usuario con `/plugin install devground@devground` — sin `npx` por proyecto. Solo skills: las configs siguen en npm y los hooks son opt-in ([matriz plugin vs npx](docs/claude-plugin.md)). | En validación (rama `claude/devground-ecc-comparison-f6tbag`) |

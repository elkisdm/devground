...
- b5ae9f7: Nuevo subcomando `devground-init machine`: instala el estandar UNA VEZ para toda la maquina en vez de repo por repo (ADR-0034).
...
- e365b10: El config de commitlint se escribe como `commitlint.config.cjs`. Exportaba con `module.exports` dentro de un `.js`, así que en cualquier proyecto con `"type": "module"` el hook `commit-msg` moría al cargarlo. Un `commitlint.config.js` que ya exista se sigue respetando, igual que antes.
- 800b76c: El estándar ya no se instala roto en proyectos de JavaScript puro. Dos fallas que solo aparecían sin TypeScript:

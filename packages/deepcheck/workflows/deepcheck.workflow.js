...
const DIMENSIONS = [
  ...
  { role: 'Auditoria', key: 'aud-premortem', ask: 'PRE-MORTEM / COMPOSICIÓN — el modo de falla que NINGUNA otra dimensión ve porque no vive en un solo archivo. Asume que esto YA falló en producción y haz la autopsia, no la defensa. (1) SUPUESTOS: lista los implícitos del diseño ("esto asume que X") e invierte cada uno ("¿y si NO-X?") hasta un efecto observable. (2) COSTURAS: ¿qué asume el componente A sobre la salida de B (rápido/no confiable) que B no garantiza? — datos re-serializados, salida de un paso barato alimentando a uno crítico, un clasificador/upstream del que depende una invariante que el componente auditado no puede defender solo. (3) PRIORIDAD plausibilidad×daño: reporta solo lo alcanzable por un actor y una secuencia REAL; descarta el teórico ("requiere primero comprometer el server" → el daño precede al hallazgo). NO dupliques aud-security (inyección/secretos/entrada) ni qa-edge (entradas raras): tu eje es la INTERACCIÓN entre componentes y los supuestos rotos, no el componente aislado.' },
  { role: 'Auditoria', key: 'aud-ui-conventions', ask: 'CONVENCIONES DE UI (SOLO flujos que generan o editan frontend — formularios, inputs, modales, botones, páginas, componentes React/Next/Tailwind; si el flujo no toca UI, NO reportes nada aquí). Rúbrica: la capa base de @devground/ui-conventions (packages/ui-conventions/skills/ui-conventions/references/base.md, §1-6) MÁS el overlay del proyecto auditado docs/ui-conventions.md si existe (el overlay GANA sobre la base en conflicto). Audita: (1) componente propio vs primitiva nativa + helper de clases (cn/twMerge, no template strings); (2) semántica de input (type/inputMode/autoComplete, sin type="number" para montos) y persistir el valor limpio/normalizado, no el formateado; (3) accesibilidad (aria-label en icon-only, label+htmlFor+id, aria-invalid/aria-describedby+role en errores, focus-visible con token, aria-hidden en svg decorativo, color por token semántico); (4) estados de error/carga (skeleton fiel, submit disabled+gerundio, error inline+foco al primero, no tragar errores de red, empty/success dedicados, error boundary con reintento); (5) microinteracciones (prefers-reduced-motion, onClose post-animación, portapapeles en try/catch); (6) región es-CL (RUT módulo 11 con helper único + error que menciona el DV, teléfono +56 9 normalizado, Intl es-CL centralizado — prefiere @devground/chile-formats). NO dupliques design-audit (estética: layout/tipografía/motion) ni aud-security. REGLA DE PROMOCIÓN — inclúyela DENTRO del rationale del hallazgo cuando aplique: si el mismo hallazgo ya aparece en la skill destilada previa de este flujo o se repite en el proyecto (≥2 veces), agrega al rationale el texto literal "PROMOVER→overlay: proponer regla en docs/ui-conventions.md"; si es una convención que converge en varios proyectos, agrega "PROMOVER→base: proponer regla en packages/ui-conventions/skills/ui-conventions/references/base.md". El key aud-ui-conventions en el ledger deja el rastro parseable para el reader futuro de dev-metrics; no construyas el reader aquí.' },
]

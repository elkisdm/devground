...
| **swift-tooling** (Swift) | `packages/swift-{format-config,package-template}/` | Primeros paquetes `@devground/swift-*` (Fase 2): config SwiftFormat+SwiftLint y plantilla `Package.swift` con isolation por capa. Artefactos copiables declarados en `index.json` (mapeo src→dest) para el futuro instalador del CLI | `swift-format-config/config/{swiftformat,swiftlint.yml}`; `swift-package-template/templates/Package.swift`; `index.json` (manifest) | 0019, 0020, 0021 |

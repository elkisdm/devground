...
| **swift-tooling** (Swift) | `packages/swift-{format-config,package-template,design-tokens}/` | Paquetes `@devground/swift-*` (Fase 2 y 4): config SwiftFormat+SwiftLint, plantilla `Package.swift` con isolation por capa, y tokens semánticos + Liquid Glass helpers. Artefactos copiables declarados en `index.json` (mapeo src→dest) para el futuro instalador del CLI | `swift-format-config/config/{swiftformat,swiftlint.yml}`; `swift-package-template/templates/Package.swift`; `swift-design-tokens/templates/{DesignTokens,Typography,Theme,LiquidGlass}.swift`; `index.json` (manifest) | 0019, 0020, 0021 |
